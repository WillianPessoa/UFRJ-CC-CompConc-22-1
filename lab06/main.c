...
* Aluno: Willian Gomes Pessoa (11621965)
* Descricao: implementa  o problema dos leitores/escritores usando variaveis de
condicao da biblioteca Pthread e adiciona prioridade para os escritores */
...
int quer_escrever = 0; // Contador de threads que querem escrever
...
void InicLeit(int id) {
  ...
  while (escr > 0 || quer_escrever > 0) {
    if (escr > 0)
      printf("L[%d] bloqueou, pois tem gente escrevendo\n", id);
    else if (quer_escrever > 0)
      printf("L[%d] bloqueou, pois tem %d querendo escrever\n", id,
             quer_escrever);
...
void InicEscr(int id) {
  ...
  escr++;
  --quer_escrever; // Não está mais na fila
  pthread_mutex_unlock(&mutex);
}
